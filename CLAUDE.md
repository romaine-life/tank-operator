...
GUI sessions use long-lived pod-side SDK runners: `agent-runner` for Claude GUI and `codex-runner` for Codex GUI. The SPA submits user turns to `POST /api/sessions/{session_id}/turns`; the backend writes a durable Cosmos `turn-queue` row keyed by `client_nonce`, and the runner claims due rows before feeding the provider SDK. Claimed rows carry `claim_id`, `claimed_by`, `claim_expires_at`, and `attempt_count`; stale claims are reclaimed inside the same still-live session pod, and terminal queue updates require the current claim. `/agent-ws` is now the live transport only, with `/timeline` as the durable replay source. This queue is scoped to browser disconnects, orchestrator rollouts, and runner-process restarts inside an otherwise live session pod. If the session pod is deleted or dies, the session is dead and its `emptyDir` workspace is gone; do not describe session-pod resurrection as a durability goal. CLI/config modes still use the legacy `/run` stream-json path. Claude `ScheduleWakeup` turns are durable delayed `turn-queue` rows with `source=schedule-wakeup` and `available_at`.
