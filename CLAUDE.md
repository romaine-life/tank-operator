...
**The session registry is the durable enumeration source.** Per `docs/migration-policy.md` and the #489 lifecycle-events ledger refactor, `backend-go/internal/sessions/Reader.List` enumerates sessions from `sessionregistry.Store` (Postgres) and hydrates each row with its matching pod's annotations + the latest `session.pod_*` lifecycle-event status. Pods are hydration data for an existing registry row, never the enumeration source on the `/api/sessions` path. A pod observed without a registry row (visible or tombstoned) is an orphan: the read path drops it from the response and ticks `tank_session_list_orphan_pod_total`. Steady-state expectation is zero orphans; non-zero means `Manager.Create` wrote a pod without a registry row, or the reaper deleted a registry row while leaving the pod running — investigate, do not auto-recover. The completion PR ("complete the session-list event migration") landed alongside the migration-guard patterns in `scripts/check-removed-chat-runtime.mjs` that block reintroduction of the four fallback paths #489 originally left live (the SPA's in-flight delete tracker, the refresh-after-mutation reseed, the reducer's placeholder synthesis on unknown session ids, and the pod-only `Reader.List` loop). See that file for the exact retired symbols.

**SPA mutation handlers do not call `refresh()`.** `deleteSession`, `createSession`, `forkSessionFromMessage`, and the glimmung-launch effect all fire their POST/DELETE and then leave the live update to the SSE stream on `/api/sessions/events`. `refresh()` itself stays — it is the first-load snapshot + the resync-required catch-up path the SSE handler calls when the cursor isn't in the durable ledger — but it is not the mutation-completion path. Per `docs/product-inspirations.md` ("User-visible run state comes from durable turn events, not local optimism"), the row's presence in the sidebar is owned by the lifecycle ledger: SSE `session.deleted` removes it; SSE `session.created` adds it (de-duped against the optimistic POST-response add the create handlers do so `activate(newId)` has a target before the SSE arrives). The prior in-flight delete tracker (a UI-state Set that drove the X-button spinner and a dimmed row) is gone for the same reason — the row vanishes when the durable event arrives; the only retained in-flight signal is a render-invisible ref that prevents double-firing the HTTP call.
...
- Every protected endpoint verifies the role claim is still `admin` or `user`, so demoting a user via the auth.romaine.life console invalidates their next exchange but the existing 7-day session JWT keeps working until it expires. Revocation that needs to be immediate happens at the auth.romaine.life session-cookie level.
- **OnboardingWall bypass**: admins skip the wall — the host installation of the GitHub App covers their MCP-github access (see "Two GitHub Apps live alongside each other" below). Non-admin users still need to install the App on their own account before installation_id lands in Postgres.
