...
Stage 3 (not yet shipped) ships the `repo-cloner` init container that reads the durable list at pod boot, mints a clone token via mcp-github, clones into `/workspace`, and writes per-repo outcome back to `sessions.clone_state jsonb` for the sidebar to render.
