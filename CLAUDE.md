...
**Repo attribution + sidebar search.** `sessions.repos` is write-once "intent" (the create-time selection). To make a session findable by *every* repo it touched — including repos the agent clones on demand mid-session, which `repos` never records — a pod-side reporter (`k8s/session-config/workspace-repo-reporter.sh`, backgrounded by the runner launch scripts since SDK-runner pods own the `/workspace` emptyDir) scans git remotes under `/workspace` and POSTs the observed `owner/name` set to `POST /api/internal/sessions/{id}/discovered-repos` (service-principal gated, same auth path as clone-state). The orchestrator folds it into `sessions.discovered_repos text[]` as a monotonic set union — a re-report of a known set is a no-op (a `@>` containment guard skips the `row_version` bump / SSE fan-out), so the reporter polls cheaply. `discovered_repos` rides every snapshot/row-update payload alongside `repos`; the sidebar filter and per-row repo chips match/render the union of the two (`frontend/src/sessionRepos.ts`). Counters: `tank_session_discovered_repos_reported_total{result}` and `tank_session_discovered_repos_dropped_total`. The capability ledger is `docs/features/session-bar/capabilities.md` → `session-repo-attribution`; the filter is a client-side view over already-reconciled rows and must never write `sidebar_position` / `row_version`.
...
- `nelsong6/mcp-azure-personal` â€” first-party personal Azure MCP server and chart; runtime naming is `mcp-azure-personal` / `azure-personal`
- `nelsong6/mcp-github` â€” custom GitHub-App-backed
- `nelsong6/mcp-k8s` â€” read-only kubectl/helm
- `nelsong6/mcp-argocd` â€” read-only ArgoCD via Dex SA-token exchange (no static API tokens)
