...
GUI sessions use long-lived pod-side SDK runners: `agent-runner` for Claude GUI and `codex-runner` for Codex GUI. The SPA submits user turns to `POST /api/sessions/{session_id}/turns`; the backend writes a durable Cosmos `turn-queue` row keyed by `client_nonce`, and the runner claims pending rows before feeding the provider SDK. `/agent-ws` is now the live transport only, with `/timeline` as the durable replay source. This queue is scoped to browser disconnects, orchestrator rollouts, and runner-process restarts inside an otherwise live session pod. If the session pod is deleted or dies, the session is dead and its `emptyDir` workspace is gone; do not describe session-pod resurrection as a durability goal. CLI/config modes still use the legacy `/run` stream-json path. ScheduleWakeup is currently handled in-process by the runner between the tool call and the re-enqueue of the wakeup turn.
