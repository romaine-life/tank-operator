...
**Programmatic CLI auth (`scripts/tank-jwt-from-az.sh`).** Agents and scripts can mint a tank session JWT by exchanging the Entra ID access token sitting in `az account get-access-token`, skipping the interactive browser flow. Three hops: `az → auth.romaine.life/api/auth/entra-exchange → tank/api/auth/exchange`. The shim caches the result at `~/.config/tank-operator/jwt` until ≤60s before its `exp`, then re-runs the chain. `scripts/tank-api.sh` is the curl wrapper that pulls from this cache and adds the `Authorization: Bearer` header. One-time setup: register an Entra app in tenant `2236b5e4-...`, set `entraExchange.{audience,tenantId}` in the auth.romaine.life helm values, sign in once interactively at auth.romaine.life so the Better Auth user row exists, then promote the row to `admin`/`user` via auth's `/admin` console (the exchange does not auto-provision). See `nelsong6/auth/README.md → "Entra ID exchange"`.
