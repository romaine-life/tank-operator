...
**Per-session repo selection.** The splash page lets the user stage up to 5 `owner/name` GitHub repo slugs at session-create time; the durable list lives on the `sessions.repos text[]` column and rides every snapshot/SSE row payload. Stage 1 (this iteration) ships the durable selection, validation (`cmd/tank-operator/handlers_repos.go`), the `GET /api/github/recent-repos` SQL surface that powers the picker's "Recent" section, the chips-and-picker UI (`frontend/src/components/RepoPicker.tsx` + `frontend/src/repos.ts`), and the `tank_session_repos_selected_total{count_bucket}` counter. The slug regex and 5-repo cap exist on both sides — `repoSlugPattern`/`maxReposPerSession` in the backend, `REPO_SLUG_PATTERN`/`MAX_REPOS_PER_SESSION` in `frontend/src/repos.ts` — and the test suites on each side assert the contract so a one-sided change surfaces. Stage 2 adds the "All repos" picker section sourced from mcp-github (cross-repo PR against `nelsong6/auth` for the on-behalf-of `actor_email` shape so the orchestrator can ask mcp-github for a user's installation repos using the IdP's identity model, not a custom side channel). Stage 3 ships the `repo-cloner` init container that reads the durable list at pod boot, mints a clone token via mcp-github, clones into `/workspace`, and writes per-repo outcome back to `sessions.clone_state jsonb` for the sidebar to render. The mode predicate `sessionModeSupportsRepos` (claude_gui / codex_gui / codex_app_server) gates the picker UI and the handler boundary: only modes whose pods provision a `/workspace` emptyDir accept a non-empty `repos[]`; CLI / config / api_key / hermes_gui reject with 400.
