...
- Break-glass writes for an approved branch-lane grant record
  `github.break_glass.push` around a governed push and
  `github.break_glass.pr_write` + `github.pull_request.open` around PR-own
  writes. These writes are governed by the agent-egress proxy (the wall); the
  in-pod broker routes that previously emitted them were retired (see
  "Branch-lane grants" below and `docs/branch-lane-grants.md`).
...
open & own its PR, scope-enforced and audited (`docs/branch-lane-grants.md`).

> **Retired (in-pod brokers → the wall).** The in-pod break-glass broker HTTP
> routes (push / PR-write) in the mcp-auth-proxy sidecar, and their per-result
> counters `tank_break_glass_push_total` / `tank_break_glass_pr_open_total` /
> `tank_break_glass_pr_write_total` plus the `TankBranchLaneBrokerErrors` alert
> and the "Branch-lane brokered push/PR outcomes" dashboard panel, were
> **removed** once the agent-egress proxy (the wall) became the GitHub boundary
> for every session. The wall mints the scoped credential and governs the
> push / PR write server-side, so a granted `git push` / `gh pr …` "just works"
> with no in-pod brokering. The durable `github.break_glass.*` /
> `github.pull_request.open` control-action audit ledger is unchanged.

What remains observable:

- The durable control-action ledger still records every successful break-glass
  write (`github.break_glass.*`, `github.pull_request.open`) and grant-lifecycle
  event, queryable via
  ...
  The dashboard "Branch-lane grants & retired-path guard" panel reads exactly those.
...
Alert (`tank-operator.branch-lane` group):
