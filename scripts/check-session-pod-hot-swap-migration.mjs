...
const CHECKS = [
  // ───────────── Checkbox 1: hot-swap of claude-runner code now works ─────────────
  ...
  },
  {
    ...
    from: "Checkbox 1: hot-swap works",
    file: "claude-container/Dockerfile",
    description: "session image bakes a launch shim at /app/claude-runner-launch-binary.sh",
    kind: "grep-present",
    pattern: /\/app\/claude-runner-launch-binary\.sh/,
  },
  {
    id: "session-image-shim-execs-node",
    from: "Checkbox 1: hot-swap works",
    file: "claude-container/Dockerfile",
    description: "baked shim's contents include `exec node /opt/claude-runner/dist/index.js` (the baked-path entrypoint the supervisor falls back to when no hot artifact is present)",
    kind: "grep-present",
    pattern: /exec\s+node\s+\/opt\/claude-runner\/dist\/index\.js/,
  },
  {
    id: "sessionmodel-volume-name",
    from: "Checkbox 1: hot-swap works",
    file: "backend-go/internal/sessionmodel/sessionmodel.go",
    description: "PodManifest declares an `claude-runner-hot` emptyDir volume",
    kind: "grep-present",
    pattern: /claude-runner-hot/,
  },
  {
    id: "sessionmodel-mount-path",
    from: "Checkbox 1: hot-swap works",
    file: "backend-go/internal/sessionmodel/sessionmodel.go",
    description: "PodManifest mounts /var/run/claude-runner-hot on the claude-runner container",
    kind: "grep-present",
    pattern: /\/var\/run\/claude-runner-hot/,
  },
  {
    id: "sessionmodel-supervisor-child-env",
    from: "Checkbox 1: hot-swap works",
    file: "backend-go/internal/sessionmodel/sessionmodel.go",
    description: "PodManifest sets GLIMMUNG_SUPERVISOR_CHILD on the claude-runner container (baked shim path)",
    kind: "grep-present",
  ...
  },
  {
    ...
    from: "Checkbox 1: hot-swap works",
    file: "backend-go/internal/sessionmodel/sessionmodel.go",
    description: "PodManifest sets GLIMMUNG_SUPERVISOR_HOT_ARTIFACT on the claude-runner container (writable shim path)",
    kind: "grep-present",
  ...
  },
  {
    ...
    from: "Checkbox 1: hot-swap works",
    file: "k8s/session-config/claude-runner-launch.sh",
  ...
  {
    id: "readme-claude-runner-source",
    from: "Checkbox 1: hot-swap works",
    file: "README.md",
    description: "agent_runner entry declares source: claude-runner/hot so dist and runner-shared are swapped together",
    kind: "grep-present",
    pattern: /"agent_runner"[\s\S]{0,900}?"source"\s*:\s*"claude-runner\/hot"/,
  },
  {
    id: "readme-claude-runner-target",
    from: "Checkbox 1: hot-swap works",
    file: "README.md",
    description: "agent_runner entry declares target: /var/run/claude-runner-hot so the hot artifact can include runner-shared",
    kind: "grep-present",
    pattern: /"agent_runner"[\s\S]{0,900}?"target"\s*:\s*"\/var\/run\/claude-runner-hot"/,
  },
  {
    id: "readme-claude-runner-bundles-shared",
    from: "Checkbox 1: hot-swap works",
    file: "README.md",
    description: "agent_runner build command copies runner-shared into the hot artifact and rewrites imports to that hot copy",
    kind: "grep-present",
    pattern: /"agent_runner"[\s\S]{0,500}?"build_command"[\s\S]{0,500}?cp -R \.\.\/runner-shared hot\/runner-shared[\s\S]{0,500}?\/var\/run\/claude-runner-hot\/runner-shared/,
  },
  {
...

  {
    ...
    description: "sessionmodel test asserts: with testEnv enabled, claude-runner container gets claude-runner-hot volumeMount + supervisor env vars",
    kind: "grep-present",
    pattern: /TestPodManifestSlotModeAttachesAgentRunnerHotSwap|testEnv[^\n]{0,200}claude-runner-hot/,
  },
...

  {
    ...
    file: "backend-go/internal/sessionmodel/sessionmodel_test.go",
    description: "sessionmodel test asserts: with testEnv disabled, claude-runner container has NO claude-runner-hot volume / mount / env vars",
    kind: "grep-present",
  ...
  },
  {
    ...
    from: "Checkbox 2: prod unchanged",
    file: "k8s/session-config/claude-runner-launch.sh",
    description: "launch script still has `exec node /opt/claude-runner/dist/index.js` as the fallback path when supervisor env vars are not set",
    kind: "grep-present",
    pattern: /exec\s+node\s+\/opt\/claude-runner\/dist\/index\.js/,
  },
  {
  ...
  },
  {
    ...
    from: "Checkbox 2: prod unchanged",
    file: "backend-go/internal/sessionmodel/sessionmodel.go",
    description: "PodManifest's hot-swap additions are gated on a testEnv-style boolean (mirrors orchestrator's pattern); the literal `claude-runner-hot` appears inside a conditional block",
    kind: "grep-present",
    pattern: /(TestEnv|testEnv|HotSwap|hotSwap)[\s\S]{0,800}?claude-runner-hot/,
  },
