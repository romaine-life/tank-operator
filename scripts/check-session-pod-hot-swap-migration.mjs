...
// Completion manifest for the session-pod agent-runner hot-swap migration.
//
...
//      the agent-runner code inside session pods (today only the orchestrator
...
//      orchestrator deployment.yaml, k8s/values.yaml's orchestrator
//      hot-swap defaults, and the existing static + backend sub-blocks of
//      README's test_slot_hot_swap entry are byte-identical to main.
//
...
const CHECKS = [
  ...
  },
  {
    ...
    from: "Checkbox 1: hot-swap works",
    file: "README.md",
    description: "agent_runner entry declares source: agent-runner/dist",
    kind: "grep-present",
    pattern: /"agent_runner"[\s\S]{0,400}?"source"\s*:\s*"agent-runner\/dist"/,
  },
  {
    id: "readme-agent-runner-target",
    from: "Checkbox 1: hot-swap works",
    file: "README.md",
    description: "agent_runner entry declares target: /var/run/agent-runner-hot/dist",
    kind: "grep-present",
    pattern: /"agent_runner"[\s\S]{0,400}?"target"\s*:\s*"\/var\/run\/agent-runner-hot\/dist"/,
  },
  {
    ...
    kind: "grep-present",
    pattern: /"agent_runner"[\s\S]{0,400}?"restart"\s*:\s*"SIGHUP"/,
  },
  {
  ...
  {
    id: "orchestrator-deployment-unchanged",
    from: "Checkbox 3: nothing else touched",
    description: "k8s/templates/deployment.yaml is byte-identical to origin/main",
    kind: "git-diff-empty",
    paths: ["k8s/templates/deployment.yaml"],
    base: "origin/main",
  },
  {
  ...
  },
  {
    ...
    from: "Checkbox 3: nothing else touched",
    description: "the static sub-block of README's test_slot_hot_swap is unchanged (only the new agent_runner sub-block is added)",
