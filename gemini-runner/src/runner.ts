...
import {
  ...
  turnUsageEmittedTotal,
  providerUsageReportTotal,
} from "./metrics.js";
import {
...
export class Runner {
  ...
  // Per-pod daily usage accumulator feeding the gemini:daily capacity snapshot.
  private usageAcc: GeminiUsageAccumulator | null = null;
  ...
  private async executeCliTurn(
    ...
    try {
      ...
      for await (const line of rl) {
        ...
        try {
          ...
          switch (event.type) {
            ...
            case "result":
              // Terminal stats frame: carries per-model token + request
              // counts. Captured here, parsed into durable usage after the
      ...
      // Usage: parse the gemini CLI result stats into a durable turn.usage
      // event and a gemini:daily capacity snapshot. Best-effort — never let a
      // usage hiccup fail an otherwise-successful turn.
      await this.recordTurnUsage(turn, resultEvent);
  ...
  // Parse the captured gemini CLI result, emit a durable turn.usage event, and
  // report a gemini:daily provider capacity snapshot to the orchestrator so the
  // SPA can surface "usage ending" for proxyless Gemini the same way it does
  // for Claude/Codex. All best-effort; failures here never fail the turn.
  private async recordTurnUsage(
    turn: GeminiAdapterTurn,
    resultEvent: unknown,
  ): Promise<void> {
    try {
      const usage = parseGeminiResultStats(resultEvent);
      if (!usage) return;
      await this.dispatch(this.adapter.turnUsage(turn, turnUsagePayload(usage)));
      turnUsageEmittedTotal.labels("terminal").inc();

      const now = Date.now();
      this.usageAcc = accumulate(this.usageAcc ?? newAccumulator(now), usage, now);
      const info = providerRateLimitInfo(
        this.usageAcc,
        this.cfg.geminiDailyRequestCap,
        now,
      );
      try {
        await reportRuntimeConfig(this.cfg, {
          model: usage.model,
          providerRateLimitInfo: info,
        });
        providerUsageReportTotal.labels("ok").inc();
      } catch (err) {
        providerUsageReportTotal.labels("error").inc();
        console.error("gemini provider usage report failed:", err);
      }
    } catch (err) {
      console.error("gemini turn usage capture failed:", err);
    }
  }
