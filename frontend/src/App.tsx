...
interface Session {
  ...
  // the server's view.
  repos: string[];
  // clone_state is the per-repo init-container outcome. Optional
  // until the cloner writes back.
...
export function App() {
  ...
  // All-repos lazy-load state. Sourced from /api/github/repos, which
  // proxies to mcp-github via an on-behalf-of token mint. The picker
  // calls onLoadAllRepos on first open; this state is the result.
  // Refreshed after a successful session create so just-used repos
  // float in the list next time the splash opens.
