...
// dependency because it reads the durable sessions.repos column.
package main
...
// clone script, while permissive enough to admit every real
// GitHub repo slug. The upstream mcp-github call is the
...
//
// Pairs with the nelsong6/auth on-behalf-of exchange (PR #43) so the
// orchestrator can present a service JWT carrying the SPA caller's
// email as `actor_email`, which mcp-github routes back to that user's
