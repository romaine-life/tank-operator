	...
	for _, tc := range cases {
		t.Run(tc.mode, func(t *testing.T) {
			...
			if tc.mode == "claude_gui" {
				// Non-wizard modes still install the session-wide Git hook template;
				// they must not seed provider onboarding/config directories.
				for _, suffix := range []string{".claude", ".codex", ".gemini"} {
					if _, err := os.Stat(filepath.Join(home, suffix)); !os.IsNotExist(err) {
						t.Errorf("non-wizard mode wrote provider config %s: %v", suffix, err)
					}
				}
			}
		})
