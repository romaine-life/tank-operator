...
// Upsert replaces the override for a scope. Empty image / git_ref strings are
// stored as NULL, so callers that want to keep an existing family override must
// send it again with the update.
