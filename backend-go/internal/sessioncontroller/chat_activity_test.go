...
func TestApplyScheduledWakeOverride(t *testing.T) {
	...
	cases := []struct {
		...
		// A runner's background_work_pending annotation parks the turn with no
		// Tank wake row, so it short-circuits the wake query and parks even
		// when the checker is nil.
