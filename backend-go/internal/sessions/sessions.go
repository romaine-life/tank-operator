...
type Info struct {
	...
	// repo-cloner writes back to sessions.clone_state. The SPA renders
	// the tooltip / status pill from this map (keyed by slug). Omitted
	// from the wire when nil to keep the snapshot lean.
