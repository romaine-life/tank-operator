...
async def mint_session_token_for_k8s_subject(sa_token: str) -> tuple[str, User]:
    ...
    NOT for session pods acting on behalf of their owner. The shared
    `claude-session` SA can only map to one email here, so using this path
    from session pods would conflate every user's traffic under a single
    principal.
    """
    if not JWT_SECRET:
        raise HTTPException(status_code=500, detail="JWT_SECRET not configured")
