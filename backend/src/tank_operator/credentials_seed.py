...
def _validate_pi(blob: dict[str, Any]) -> None:
    """Sanity-check Pi's auth.json enough to avoid storing junk.

    Pi supports multiple providers and auth methods, so the exact shape is
    intentionally less constrained than Codex's ChatGPT-only file. The
    bootstrap only needs a valid JSON object to copy into ~/.pi/agent/auth.json.
    """
    if not blob:
        raise CredentialsSeedError("pi auth.json is empty")
...
async def harvest_pi_and_save(namespace: str, pod_name: str) -> None:
    ...
    if not raw:
        raise CredentialsSeedError(
            "pi auth.json is empty or missing — complete `/login` in the "
            "Pi session terminal first"
        )
    try:
        blob = json.loads(raw)
    except json.JSONDecodeError as e:
