...
Some sessions are created with `TANK_RESTRICTED_GIT=true` (governed Git). Every
Tank session routes its GitHub traffic through the agent-egress proxy (the
wall) — it is the one governed GitHub egress path, and the workflow is:
...
- Your repos are checked out on a Tank-owned branch (`tank/session/<id>/<repo>`) with
  ...
  `git push`** — the wall mints the GitHub write token server-side, records every
  action, confines you to your own branch, and starts CI/mergeability watching.
  There is no separate publish step; just commit and push, and the PR updates.
- You CANNOT push to `main` or merge a PR — the wall rejects both with HTTP 403.
...
If `TANK_RESTRICTED_GIT` is not true, use the normal repo workflow available in
the session.
